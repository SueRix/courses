...
class _EnsurePackagesDiscovered(_expand.EnsurePackagesDiscovered):
    ...
    def __enter__(self):
        ...
        ``package_dir`` that are missing in ``templates`` are copied from ``setuptools_cfg``.
        """
        ...
        # Set `name`, `py_modules` and `packages` in templates to short-circuit
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        ``package_dir`` are missing in ``setuptools_cfg``, copy from ``templates``.
        """
